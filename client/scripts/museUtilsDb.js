...
(function(pPublicApi) {
    ...
    var getQtSqlQueryStats = function(pSqlQuery) {
        ...
        var vReturnText = "toString: " + pSqlQuery.toString() + "\n" +
            "first: " + pSqlQuery.first() + "\n" +
            "isActive: " + pSqlQuery.isActive() + "\n" +
            "isValid: " + pSqlQuery.isValid() + "\n" +
            "isForwardOnly: " + pSqlQuery.isForwardOnly() + "\n" +
            "isSelect: " + pSqlQuery.isSelect() + "\n" +
            "size: " + pSqlQuery.size() + "\n" +
            "numRowsAffected: " + pSqlQuery.numRowsAffected() + "\n" +
            "lastError: " + JSON.stringify(pSqlQuery.lastError());

        return vReturnText;
    };

    var getJsonQueryResult = function(pQueryText, pMetaSqlGroup, pMetaSqlName, pParams) {
        ...
        for (var i = 0; i < queryRecord.count(); i++) {
            functionString = 
                functionString + queryRecord.fieldName(i) + ': this.value("' + 
    ...
    var setDbTransVariable = function(pPackage, pVariable, pValue) {
         ...
         try {
             ...
             if(tranVarQry.first() && MuseUtils.isTrue(tranVarQry.value("result"))) {
                 return;
             } else {
                 throw new MuseUtils.DatabaseException(
                     "musextputils",
    ...
    var setDbSessionVariable = function(pPackage, pVariable, pValue) {
        ...
        try {
            ...
            if(sessVarQry.first() && MuseUtils.isTrue(sessVarQry.value("result"))) {
                return;
            } else {
                throw new MuseUtils.DatabaseException(
                    "musextputils",
    ...
    var getDifferencedData = function(pNewMappedData, pOldMappedData, pKeyColumnName) {
        var returnData = {};
        ...
        if(MuseUtils.realNull(pOldMappedData) === null) {
            for (var tmpNewField in pNewMappedData) {
                if(pNewMappedData.hasOwnProperty(tmpNewField) && 
            ...
            for (var tmpDiffField in pNewMappedData) {
                if(pNewMappedData.hasOwnProperty(tmpDiffField) && 
                    ...
                    if((pNewMappedData[tmpDiffField] != pOldMappedData[tmpDiffField]) || 
                        tmpDiffField == pKeyColumnName) {
                        returnData[tmpDiffField] = pNewMappedData[tmpDiffField];
                    }
                }
            }
        }
    ...
    pPublicApi.executeDbQuery = function(pMetaSqlGroup, pMetaSqlName, pParams) {
        try {
            // Validate parameters
            if(MuseUtils.realNull(pMetaSqlGroup) === null || 
                MuseUtils.realNull(pMetaSqlName) === null) {
            ...
            return getJsonQueryResult(null, pMetaSqlGroup, pMetaSqlName, pParams);
        } catch(e) {
            throw new MuseUtils.ApiException(
                "musextputils",
                "There was an error during the execution of an API call.",
    ...
    pPublicApi.setDbSessionVariable = function(pPackage, pVariable, pValue) {
        try {
            //Validate our parameters
            if(MuseUtils.realNull(pPackage) === null) {
                throw new MuseUtils.ParameterException(
    ...
    pPublicApi.getDbVariable = function(pPackage, pVariable, pType) {
        try {
            ...
            } else if(MuseUtils.realNull(pType) === null ||
    ...
    pPublicApi.getDifferencedData = function(pNewMappedData, pOldMappedData, pKeyColumnName) {
        var funcParams = {
        ...
        try {
            return getDifferencedData(pNewMappedData, pOldMappedData, 
                pKeyColumnName);
        } catch(e) {
            throw new MuseUtils.ApiException(
                "musextputils",
